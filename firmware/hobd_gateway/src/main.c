...
{
    //
    can_tx_heartbeat_frame.state_id = 1;
    ...
    //
    can_tx_response_frame.state_id = 2;
    ...
    //
    can_tx_dtable_frame.state_id = 3;
...
{
    //
    can_rx_command_frame.state_id = 4;
...
static void process_can_command( void )
{
    ...
    // get current time
    const uint32_t now_time = time_get_ms();
    ...
    if( ret == 2 )
    {
        ...
        can_rx_command_frame.timestamp = now_time;

        DEBUG_PRINTF( "can_rx 0x%lX\r\n", can_rx_command_frame.id );

        // handle supported frames
        if( can_rx_command_frame.id == HOBD_CAN_ID_COMMAND )
        {
            // TODO
        }
    }
