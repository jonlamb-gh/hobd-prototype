...
//_____ D E C L A R A T I O N S ________________________________________________
    ...
    uint8_t uart_selected;
#endif

#if ((UART_BAUDRATE == VARIABLE_UART_BAUDRATE) || (UART_BAUDRATE == UART_AUTOBAUD))
    #ifndef USE_TIMER16
        #error You must define USE_TIMER16 to TIMER16_1 or TIMER16_3 or BOTH_TIMER16 in "board.h" file
    #elif (USE_TIMER16 ==  BOTH_TIMER16)
        uint8_t timer16_selected;
    #endif
#endif
...
{
    uint16_t t_meas_0, t_meas_1, t_meas_2, t_meas_3, t_meas_4;
    uint16_t t_min, t_max;

    if (baudrate != 0)
    {
        // UBRRN Value (rounded to nearest division) with U2Xn=1
        t_meas_4 = (uint16_t)(( (((((uint32_t)FOSC*1000)<<1)/((uint32_t)baudrate*8))+1)>>1)-1);
    }
...
//
    ...
    #if (USE_UART ==  UART_0)
        UBRR0H  = (uint8_t)((t_meas_4)>>8);
        UBRR0L  = (uint8_t) (t_meas_4)    ;
        UCSR0A |=  (1<<U2X0);
    #elif (USE_UART ==  UART_1)
        UBRR1H  = (uint8_t)((t_meas_4)>>8);
        UBRR1L  = (uint8_t) (t_meas_4)    ;
        UCSR1A |=  (1<<U2X1);
