...
#endif
#ifndef NO_UART              // No UART Defintion (do not use it in "board.h")
#define NO_UART     0xFE
#endif
    // ----------
#ifndef USE_UART
...
#   elif USE_UART == UART_0
    ...
    //!< ==============================
        // Macros
#       define Uart_set_ubrr(baudrate) { UBRR0H = (uint8_t)((((((((uint32_t)FOSC*1000)<<1)/((uint32_t)baudrate*8))+1)>>1)-1)>>8); \
                                         UBRR0L = (uint8_t)(( (((((uint32_t)FOSC*1000)<<1)/((uint32_t)baudrate*8))+1)>>1)-1)    ; \
                                         UCSR0A |=  (1<<U2X0); }
...
#   elif USE_UART == UART_1
    ...
    //!< ==============================
        // Macros
#       define Uart_set_ubrr(baudrate) { UBRR1H = (uint8_t)((((((((uint32_t)FOSC*1000)<<1)/((uint32_t)baudrate*8))+1)>>1)-1)>>8); \
                                         UBRR1L = (uint8_t)(( (((((uint32_t)FOSC*1000)<<1)/((uint32_t)baudrate*8))+1)>>1)-1)    ; \
                                         UCSR1A |=  (1<<U2X1); }
...
#   elif USE_UART  == BOTH_UART
    ...
    //!< =====================================
        extern uint8_t uart_selected;      // $$$-- EXTERNAL DECLARATION --$$$
        #define Uart_select(uart_num)   (uart_selected=uart_num)
        // Macros
#       define Uart_set_ubrr(baudrate)  ((uart_selected==UART_0)? \
                                            (UBRR0H  = (uint8_t)((((((((uint32_t)FOSC*1000)<<1)/((uint32_t)baudrate*8))+1)>>1)-1)>>8), \
                                             UBRR0L  = (uint8_t)(( (((((uint32_t)FOSC*1000)<<1)/((uint32_t)baudrate*8))+1)>>1)-1)    , \
                                             UCSR0A |=  (1<<U2X0) )\
                                            : \
                                            (UBRR1H  = (uint8_t)((((((((uint32_t)FOSC*1000)<<1)/((uint32_t)baudrate*8))+1)>>1)-1)>>8), \
                                             UBRR1L  = (uint8_t)(( (((((uint32_t)FOSC*1000)<<1)/((uint32_t)baudrate*8))+1)>>1)-1)    , \
                                             UCSR1A |=  (1<<U2X1) )                                                     )
...
//------------------------------------------------------------------------------
//! Set UART baudrate.
//! 1) If "UART_BAUDRATE == xxxxx" in "board.h" file, all the UARTs defined
//!    will be set with this value (computation made by the C pre-processor).
//! 2) If "UART_BAUDRATE == UART_AUTOBAUD" in "board.h" file, an automatic
//!    baudrate detection will be maded on all the UARTs defined.
//! 3) If "UART_BAUDRATE == VARIABLE_UART_BAUDRATE" in "board.h" file,
...
//! 4) If "UART_BAUDRATE" is not defined in "board.h" file, "UART_BAUDRATE"
...
//!
extern  uint8_t uart_set_baudrate(uint32_t baudrate);
#endif
